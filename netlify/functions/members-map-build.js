...
function foldMember(state, m, weekAgo) {
  ...
  b.total++;
  if (t) b[t]++;
  // ⚠️ mmb-v35 CRASH FIX: byZip buckets carried forward from cycles created before v26
  // have no memberIds array, so this .push() threw "Cannot read properties of undefined
  // (reading 'push')", killing the whole build before the snapshot/new7 could publish.
  // That is why v34's ceiling fix ran (log showed 76%) but new7 never updated.
  if (!b.memberIds) b.memberIds = [];
  b.memberIds.push(id);
...
async function build(opts) {
  ...
  if (!warmOnly) {
    try {
      ...
      if (needTopScan) {
        ...
        // fine: from there, step UP to the true highest real id — but only a SHORT
        // window (25 ids, the coarse step size). ⚠️ mmb-v36: v34/35 stepped up all the
        // way to ID_CEILING, which with an 885-id empty gap above the real top meant
        // ~900 single 300ms reads — blowing the ENTIRE time budget every wake. So the
        // top was never finalized, never cached, rediscovery repeated forever, and the
        // main scan never advanced (stuck at 76%). The real top is within one coarse
        // step (25) above `found`, so only probe that far.
        if (found) {
          let top = found;
          const fineTop = Math.min(ID_CEILING, found + 25);
          for (let id = found + 1; id <= fineTop; id++) {
            if (Date.now() > tailDeadline) break;
            const m = await fetchMemberById(id);
            await sleep(REQUEST_DELAY_MS);
            if (m) top = id;
  ...
  // ⚠️ mmb-v38 CRITICAL FIX: this write held a STALE copy of the cache and was
  // clobbering the __topId that the tail rediscovery saved (4315) back to the old
  // ceiling (3950). That made every wake re-run rediscovery, burn the whole budget,
  // and never complete a pass — the map stayed frozen. Re-read the freshest __topId
  // from the store and preserve it so this write can never undo the correction.
  try {
    const freshest = JSON.parse(await store.get(KEY_ZIPCACHE));
    if (freshest && freshest.__topId && freshest.__topId > (cache.__topId || 0)) {
      cache.__topId = freshest.__topId;
    }
  } catch (e) {}

...
//
// at-v25: AN ON-SCREEN EVENT TRAIL FOR MOBILE. A phone has no console,
// and four versions have now been spent guessing at a failure from its
// symptoms. Add ?rdcdebug=1 to the homepage URL and every speech event -
// start, result, error, end, watchdog, extract - is listed under the
// recorder in order, on screen, readable off the device.
// Off without the flag: it renders nothing and costs nothing.
//
// 📌 WHEN A SURFACE CANNOT BE INSPECTED, INSTRUMENT THE SURFACE. Cheaper
// than a fifth guess, and it gives an answer rather than another theory.
//
...
(function () {
  ...
  // at-v25: AN ON-SCREEN EVENT TRAIL, because a phone has no console and
  // guessing from symptoms has now cost four versions. Add ?rdcdebug=1 to
  // the homepage URL and every speech event is listed under the recorder,
  // on screen, in order. Off otherwise - it renders nothing and costs
  // nothing.
  var DBG = false;
  try { DBG = /[?&]rdcdebug=1/.test(window.location.search); } catch (e) {}
  var dbgLog = [];
  function dbg(msg) {
    if (!DBG) return;
    var t = new Date();
    dbgLog.push(t.toTimeString().slice(3, 8) + " " + msg);
    if (dbgLog.length > 14) dbgLog.shift();
    var el = document.getElementById("rt-dbg");
    if (el) el.innerHTML = dbgLog.map(esc).join("<br>");
  }
  function dbgHtml() {
    if (!DBG) return "";
    return '<div id="rt-dbg" style="background:#111;color:#7fe0c8;font:11px/1.5 monospace;' +
      'padding:10px 12px;border-radius:8px;margin:0 0 12px;text-align:left;white-space:pre-wrap;' +
      'word-break:break-word;">' + (dbgLog.length ? dbgLog.map(esc).join("<br>") : "waiting...") + '</div>';
  }
  ...
  function renderForm() {
    ...
    dbg("wired. inline=" + inlineRec + " active=" + voice.active + " chars=" + String(voice.transcript || "").length);
      ...
      '<div style="' + S.card + '">' +
        ...
        (voice.busy
          ? '<div style="' + S.busyBox + '">' +
            ...
            '</div>'
          : dbgHtml() + '<div id="rt-live" style="' + (voice.active ? S.liveOn : S.live) + '">' + (esc(voice.transcript + voice.interim) || '<span style="color:#9aa8b8;">Your words will show up here.</span>') + '</div>') +
        (voice.busy
          ? ""
          : (voice.active
  ...
  function startRec() {
    ...
    r.onresult = function (ev) {
      ...
      dbg("onresult chars=" + String(voice.transcript + voice.interim).length);
    ...
    var watchdog = setTimeout(function () {
      ...
      dbg("WATCHDOG fired, no audio in 8s");
      voice.active = false;
      try { r.stop(); } catch (e) {}
    ...
    r.onstart = function () { dbg("onstart"); voice.status = "Listening. Take your time."; paint(); };

    r.onerror = function (ev) {
      dbg("onerror: " + (ev && ev.error ? ev.error : "unknown"));
      heard = true;
      clearTimeout(watchdog);
      voice.active = false;
    ...
    r.onend = function () {
      dbg("onend. active=" + voice.active + " chars=" + String(voice.transcript || "").length);
      clearTimeout(watchdog);
    ...
    dbg("start() continuous=" + r.continuous);
    try { r.start(); } catch (e) { dbg("start threw: " + (e && e.message)); voice.active = false; voice.status = "We could not start the microphone. Type it instead."; }
    paint();
  }
  ...
  function extractVoice(transcript) {
    dbg("extract called, chars=" + String(transcript || "").length);
    ...
    fetch(VOICE, {
      ...
      dbg("extract returned ok");
      voice.busy = false;
      inlineRec = false;
      applyExtraction(d, transcript);
    }).catch(function (e) {
      dbg("extract FAILED: " + (e && e.message));
      voice.busy = false;
      inlineRec = false;
